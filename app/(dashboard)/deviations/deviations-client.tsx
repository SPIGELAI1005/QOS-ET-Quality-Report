...
export function DeviationsClient() {
  ...
  useEffect(() => {
    fetch("/api/deviations")
      ...
      .then((data) => {
        if (Array.isArray(data?.deviations)) setDeviations(data.deviations);
      })
      .catch((e) => console.error("[Deviations] Failed to load deviations from API:", e));
  }, []);
  ...
  useEffect(() => {
    fetch("/api/plants")
      ...
      .catch((e) => console.error("[Deviations] Failed to load plants from API:", e));
  }, []);
    ...
    try {
      const selectedSitesForAI =
        filters.selectedPlants.length > 0
          ...
          : Array.from(new Set(deviationsMonthlySiteKpisForAi.map((k) => k.siteCode))).sort();

      const selectedMonthsForAI = Array.from(new Set(deviationsMonthlySiteKpisForAi.map((k) => k.month))).sort();
  ...
  }, [deviationsMonthlySiteKpisForAi, filters, aiSummaryErrorType]);
  ...
  const deviationsByMonthPlant = useMemo(() => {
      ...
      <div className="flex-1 space-y-6">
            ...
            <div className="space-y-4">
              <Card className="glass-card-glow" style={{ borderColor: "#9E9E9E", borderWidth: "2px" }}>
                <CardContent className="p-6 flex flex-col">
                  <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">D Notifications</p>
                  <div className="text-3xl font-bold mt-1">{totalDNotifications.toLocaleString("de-DE")}</div>
                  <p className="text-xs text-muted-foreground mt-1">D1 / D2 / D3 deviations</p>
              ...
              <Card className="glass-card-glow" style={{ borderColor: "#9E9E9E", borderWidth: "2px" }}>
                <CardContent className="p-6 flex flex-col">
                  <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">D Notification Status</p>
                  <div className="text-3xl font-bold mt-1">
                    {closed.toLocaleString("de-DE")} / {inProgress.toLocaleString("de-DE")}
                  </div>
                  ...
                  <div className="flex items-start justify-between mb-3 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        disabled={aiSummaryLoading || deviationsMonthlySiteKpisForAi.length === 0}
                  ...
                  <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                    {aiSummaryLoading ? (
                      ...
                      <div className="space-y-1.5">
                        ...
                        {deviationsMonthlySiteKpisForAi.length === 0 ? (
        ...
        <div className="space-y-6">
              ...
              {deviationsByMonthPlant.data.length > 0 && deviationsByMonthPlant.sites.length > 0 ? (
                <>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={deviationsByMonthPlant.data} margin={{ top: 30, right: 30, left: 20, bottom: 5 }}>
                      ...
                      />
                      {deviationsByMonthPlant.sites.map((site, index) => {
                        const isLast = index === deviationsByMonthPlant.sites.length - 1;
                        return (
                          <Bar
                            key={site}
                            dataKey={site}
                          ...
                          >
                            {isLast && (
                              <LabelList
                                ...
                                fontWeight="500"
                                offset={10}
                                formatter={(v: number) => (v ? v.toLocaleString("de-DE") : "")}
                              />
                            )}
                          </Bar>
                        );
                  ...
                  <PlantLegend sites={deviationsByMonthPlant.sites} />
                </>
              ) : (
          ...
          <Card className="glass-card-glow chart-container">
            ...
            <CardContent>
              {deviationStatusByMonth.length > 0 ? (
                <>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={deviationStatusByMonth} margin={{ top: 30, right: 30, left: 20, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255, 255, 255, 0.1)" />
                      <XAxis dataKey="month" stroke="rgba(255, 255, 255, 0.5)" />
                      <YAxis stroke="rgba(255, 255, 255, 0.5)" />
                      <Tooltip
        ...
        <Card className="glass-card-glow chart-container">
          <CardHeader>
            ...
            <CardDescription className="mt-2">Deviations metrics on this page are calculated from the following data sources</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold mb-1">Deviations Notifications</h4>
                  <p className="text-sm text-muted-foreground mb-2">Deviations Notifications_PS4.XLSX</p>
                  <p className="text-sm mb-3">D1/D2/D3 notifications</p>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold mb-1">Deviations Status Reference</h4>
                  <p className="text-sm text-muted-foreground mb-2">Deviations Notifications_STATUS_PS4.XLSX</p>
